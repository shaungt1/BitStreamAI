...
# minimal torchvision with ops.nms -> pure PyTorch NMS
...
# INPUT (your CSI->MediaMTX stream exposed via RTSP)
...
# Force CPU so it doesn't die on missing CUDA
DEVICE = "cpu"

print("Loading model on", DEVICE)
model = YOLO(MODEL_PATH)

print("Opening input:", RTSP_IN)
cap = cv2.VideoCapture(RTSP_IN, cv2.CAP_FFMPEG)
if not cap.isOpened():
    print("ERROR: failed to open input", RTSP_IN)
    sys.exit(1)

print("Starting FFmpeg publisher ->", RTSP_OUT)
ffmpeg_cmd = [
    "ffmpeg",
    "-loglevel","error",
    "-re",
...
try:
    while True:
        ...
        # CPU inference (works even with wrong Torch CUDA)
